...
class BipedalWalker(gym.Env, EzPickle):
    ...
    def __init__(self, render_mode: Optional[str] = None, hardcore: bool = False, noise: float = 0.0,
                 slope: float = 0.0):
    ...
    def _generate_terrain(self, hardcore):
        ...
        for i in range(TERRAIN_LENGTH - 1):
            ...
            self.terrain.append(t)
            color = (102, 153, 76)
            poly += [(poly[1][0], TERRAIN_DEPTH), (poly[0][0], TERRAIN_DEPTH)]
    ...
    def step(self, action: np.ndarray):
        ...
        state += [l.fraction for l in self.lidar]
        assert len(state) == 24
        self.scroll = -pos.x * SCALE + VIEWPORT_W / 5
        self.scroll_y = -pos.y * SCALE + VIEWPORT_H / 2
        shaping = (
            130 * pos[0] / SCALE
    ...
    def render(self):
        ...
        for poly, x1, x2 in self.cloud_poly:
            ...
            pygame.draw.polygon(
                ...
                color=(255, 255, 255),
                points=[
                    (p[0] * SCALE + self.scroll, p[1] * SCALE + self.scroll_y) for p in poly
                ],
            )
            gfxdraw.aapolygon(
                self.surf,
        ...
        for poly, color in self.terrain_poly:
            ...
            scaled_poly = []
            for coord in poly:
                scaled_poly.append([(coord[0] * SCALE) + self.scroll, (coord[1] * SCALE) + self.scroll_y])
        ...
        if i < 2 * len(self.lidar):
            ...
            if hasattr(single_lidar, "p1") and hasattr(single_lidar, "p2"):
                pygame.draw.line(
                    self.surf,
                    color=(255, 0, 0),
                    start_pos=(single_lidar.p1[0] * SCALE + self.scroll, single_lidar.p1[1] * SCALE + self.scroll_y),
                    end_pos=(single_lidar.p2[0] * SCALE + self.scroll, single_lidar.p2[1] * SCALE + self.scroll_y),
                    width=1,
                )
            ...
            for f in obj.fixtures:
                ...
                if type(f.shape) is circleShape:
                    pygame.draw.circle(
                        self.surf,
                        color=obj.color1,
                        center=trans * (f.shape.pos[0] - self.scroll, f.shape.pos[1] - self.scroll_y) * SCALE,
        ...
        # Drawing the flag:
        # Pole:
        flagy1 = TERRAIN_HEIGHT * SCALE + self.scroll_y
        flagy2 = flagy1 + 50
        x = TERRAIN_STEP * 3 * SCALE + self.scroll
        pygame.draw.aaline(
